...
def build_policy_net(input_size_, output_size):
    ...
    policy_net = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(int(output_size * 2), activation="relu", kernel_regularizer=regularizers.l2(0.001)),
        tf.keras.layers.Dropout(0.1),
        tf.keras.layers.Dense(output_size, activation="softmax")
    ])

    # Compile the policy network
    # policy_net.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.008), loss="categorical_crossentropy", metrics=['accuracy'])
    # SGD was better than Adam
    print('SGD')
    policy_net.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=0.02, momentum=0.9), loss="categorical_crossentropy", metrics=['accuracy'])
    return policy_net
...
def train_policy_net(policy_net, train_data, train_labels, test_data, test_labels, num_epochs=10000, batch_size=1024):
    ...
    for i in range(num_epochs):
        ...
        x = train_data[idxs]
        y = train_labels[idxs]
        policy_net.fit(x, y, epochs=1, validation_split=0, verbose=0)
        ...
        if i % 100 == 0:
            ...
            # calculate train and test percentage differences to figure out overfitting
            loss_percent_diff = calculate_percentage_difference(loss, test_loss)
            accu_percent_diff = calculate_percentage_difference(train_accuracy, test_accuracy)
            print(f"Step {i}\ttrain_loss:\t{loss:.4f}\ttest_loss:\t{test_loss:.4f}\ttrain_accuracy:\t{train_accuracy:.4f}\ttest_accuracy:\t{test_accuracy:.4f}\tloss_percentdiff:\t{loss_percent_diff:.2f}%\taccu_percentdiff:\t{accu_percent_diff:.2f}%")
    return policy_net
...
if __name__ == "__main__":
    ...
    exp_data = D['exp_data']
    Train_nonoptml_data = D['train_policy_df_nonop']
    Test_nonoptml_data = D['test_df_nonop']
    Train_demo_observations = np.asarray(Train_nonoptml_data[D['UPDATED_STATE_COLs']])
    Train_demo_actions = np.asarray(Train_nonoptml_data[D['ACTION_COLs']])
    Test_demo_observations = np.asarray(Test_nonoptml_data[D['UPDATED_STATE_COLs']])
    Test_demo_actions = np.asarray(Test_nonoptml_data[D['ACTION_COLs']])
    obs_dim = len(Train_demo_observations[1])
    act_dim = len(Train_demo_actions[1])

    train_data, test_data, train_labels, test_labels = Train_demo_observations, Test_demo_observations, Train_demo_actions, Test_demo_actions
    oversampled_train_data, oversampled_train_labels, oversampled_action_labels = handle_data_imbalance(train_data, train_labels)

    # Build and train the policy network
    policy_net = build_policy_net(obs_dim, act_dim)
    policy_net = train_policy_net(policy_net, oversampled_train_data, oversampled_train_labels, test_data, test_labels)

    # Saving the model
    policy_net.save('./BC_models/nonopt_bc_model.h5')

    # UPDATED TO CREATE TRAJECTORIES
    ...
    df_altered = BCUtils.create_trajectories('NOP', merged_df, policy_net, state_cols, action_cols)
    df_altered.to_csv('./BC_trajectories/BC_NonOPT_Trajectories.csv', index=False)
